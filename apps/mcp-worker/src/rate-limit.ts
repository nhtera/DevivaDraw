/**
 * Simple per-key fixed-window token bucket, in-memory per Worker isolate — copied from
 * `apps/collab-server/src/rate-limit.ts` (pattern reuse, not process reuse — see the MCP plan).
 * Abuse-deterrence, not a security boundary (this Worker stores nothing, so the blast radius of
 * abuse is compute, not data): requests from the same
