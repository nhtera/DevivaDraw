/**
 * Minimal "which arrow's path passes near this point" hit test for the dev harness's
 * double-click-to-edit-arrow-label wiring — the same "safe over-inclusion via a cheap per-segment
 * distance test, topmost z-order wins" trade-off `find-bindable-container-at-point.ts` makes for
 * shapes, adapted for a multi-point path instead of a filled bbox (an arrow has no interior to hit).
 */
