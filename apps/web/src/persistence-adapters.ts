/**
 * Thin, DOM-only adapters the dev harness wires the engine's persistence/export functions through —
 * download/open file plumbing, and a real `<canvas>`-backed `ExportRenderTarget` for PNG export.
 * Deliberately dumb: every actual decision (what to serialize, how to validate, how to render) lives
 * in `@deviva-draw/engine`; this file only knows how to talk to the browser.
 */
 ...
 * explicit feature-detection branch, not a silent no-op, per this phase's spec.
 */
