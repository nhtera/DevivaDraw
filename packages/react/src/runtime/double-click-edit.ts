...
export function attachDoubleClickToEditListener(options: DoubleClickEditOptions): () => void {
  ...
  const handleDoubleClick = (event: MouseEvent) => {
    openEditAtClientPoint(options, event.clientX, event.clientY);
  };
...
/**
 * The double-click-to-edit dispatch itself, callable outside a native `dblclick` — the touch layer
 * invokes it from its own double-tap detection (iOS Safari never synthesizes `dblclick` from
 * double-taps). Returns `true` only when a TEXT editor opened (table cell, bound/arrow label,
 * standalone text) — the signal a touch caller uses to bootstrap the on-screen keyboard while still
 * inside the gesture; embed activation, group drill-in and crop return `false` (no keyboard wanted).
 */
export function openEditAtClientPoint(options: DoubleClickEditOptions, clientX: number, clientY: number): boolean {
  const { container, scene, toolStateMachine, selectToolName, editSession, textMeasurer, getCamera, styleState, selection } = options;
  if (toolStateMachine.getActiveToolName() !== selectToolName) return false;
  const rect = container.getBoundingClientRect();
  const camera = getCamera();
  const scenePoint = screenToScene({ x: clientX - rect.left, y: clientY - rect.top }, camera);
