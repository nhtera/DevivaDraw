...
export function PropertiesPanel(props: PropertiesPanelProps) {
  ...
  return (
    <div style={{ ...panelStyle, position: "absolute", top: 12, right: 12, width: 220, padding: 12, display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      />
      <StyleSection label={t("panel.fill")} value={style.fillStyle} options={FILL_STYLE_OPTIONS.map((v) => ({ value: v, label: t(`styleValue.${v}`) }))} onChange={(v) => apply({ fillStyle: v })} />
      <StyleSection
        ...
        options={Object.keys(STROKE_WIDTH_LEVELS).map((key) => ({ value: key, label: t(`styleValue.${key as "thin" | "bold" | "extra-bold"}`) }))}
      ...
      <StyleSection
        ...
        options={STROKE_STYLE_OPTIONS.map((v) => ({ value: v, label: t(`styleValue.${v}`) }))}
      ...
      <StyleSection
        ...
        options={Object.keys(SLOPPINESS_LEVELS).map((key) => ({ value: key, label: t(`styleValue.${key as "architect" | "artist" | "cartoonist"}`) }))}
        onChange={(key) => apply({ roughness: SLOPPINESS_LEVELS[key as keyof typeof SLOPPINESS_LEVELS] })}
      />
      <StyleSection
        label={t("panel.edges")}
        value={roundnessKey(style.roundness)}
        options={["sharp", "round"].map((key) => ({ value: key, label: t(`styleValue.${key as "sharp" | "round"}`) }))}
