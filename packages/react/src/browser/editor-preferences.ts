 ...
 * may attach to shapes, and whether they snap onto a shape's edge midpoints. Same shape as
 * `input-device-preference.ts` (validated JSON object in localStorage + a module-level store bridged
 * to React by `useSyncExternalStore`), and deliberately ONE module holding all three rather than a
 * file per preference: they share a storage key, a parse routine and a menu section, and three
 * near-identical copies of this file would drift.
 *
...
export const DEFAULT_EDITOR_PREFERENCES: EditorPreferences = { selectOnMode: "auto", arrowBinding: true, snapMidpoints: true };
