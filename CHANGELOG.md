...
- **Vietnamese text renders in the hand-drawn face, not half in Helvetica.** The bundled font covered
  Latin-1 only — 212 codepoints, no Vietnamese and no Latin Extended at all — so a sentence with a
  diacritic in it came out half sketchy and half sans-serif, on the canvas and baked into every PNG
  and SVG export. In a product that ships a Vietnamese locale, that was every other sentence. The
  bundled face is now Patrick Hand (SIL OFL 1.1), subset to Latin-1, Latin Extended-A/B and the full
  Vietnamese precomposed block, and the artifact is generated by a committed script
  (`pnpm --filter @deviva-draw/engine font:build`) from a committed, hash-verified source font
  instead of being an unreproducible blob. Text is stored unwrapped and re-measured at render time,
  so nothing stored is invalidated by the change — but the new face is narrower than the old one, so
  existing text re-wraps a little tighter than it did.
- **Find searches the whole document, not just the page you are looking at.** Multi-page documents
  arrived in 0.10 while find stayed scene-scoped, so Cmd+F quietly answered "no matches" for text one
  page away. Matches now come from every page in page order, the panel names the page each match is
  on, and stepping onto a match on another page switches to it before revealing. Abandoning the
  search with Escape puts you back on the page you started from. Frame names are searchable too.
- **An oversized image is resized and inserted, instead of silently refused.** The 10 MB cap dated
  from when every byte went into the localStorage autosave; image bytes moved to IndexedDB in 0.10,
  so an ordinary phone photo was being turned away for a reason that no longer existed — and the
  refusal was a `console.warn`, meaning the user saw nothing happen at all. Images over the budget
  are now re-encoded smaller (8000px on the longest edge) and inserted, with an on-screen notice
  saying so; images that genuinely cannot be inserted get a message naming the limit they hit.
- **A drag abandoned by a page switch no longer half-applies.** Switching pages mid-drag tore the
  runtime down without ending the gesture, leaving the partial move written to the scene while the
  history batch that would have recorded it was discarded — the shape moved, with nothing to undo it
  back. Tearing the input pipeline down now cancels any gesture still in flight, which restores the
  pre-drag positions. This was reachable through the new cross-page find, but the hole was in every
  teardown path.

### Added

- **Equal-spacing guides.** Dragging a shape between two others now snaps to equal gaps and draws
  both spans with their measured distance, and dragging past an evenly spaced pair repeats that
  spacing — so a row built one shape at a time comes out actually even, not approximately even.
  Alignment snap still wins wherever both apply, and a group's own internal spacing is never offered
  as a gap to an unrelated shape. Rides the existing "snap to objects" preference (Alt+S), and is off
  whenever grid snap is on.
- **A PNG, JPEG, GIF or WebP decompression bomb is refused before it is decoded.** Dimensions are now
  read from the file's own header bytes — sniffed from the content, not the declared type, so
  renaming the file does not dodge the check — and anything declaring more than 256 megapixels is
  rejected without ever allocating the buffer. A few hundred KB of PNG can declare a multi-gigapixel
  canvas, and the old byte-length check ran before a decode that had already materialised it. AVIF
  and HEIC are not yet covered; they are decoded as they always were.

### Changed

- **Breaking (embedders):** `usePasteAndDrop`'s `onInsertError(error: unknown)` is replaced by
  `onInsertOutcome(outcome: ImageInsertOutcome)`. The old callback could not express the outcome that
  matters most now — "it worked, but I resized it" — and its only implementations logged to the
  console. Pass the exported `downscaleImage` adapter as `downscale` to opt into
  resize-instead-of-refuse; an embedder that supplies none keeps exactly the previous behaviour.
- The bundled font is documented in `LICENSE-THIRD-PARTY`, with its upstream source, SHA-256 and
  license text committed beside it in `packages/engine/assets/`.
- **MCP text measurement re-calibrated for the new face.** The canvas-free fallback measurer
  approximated every glyph at 0.6 of the font size, calibrated against the previous, wider font.
  Measured from the new face's own metrics it is 0.36–0.40, so headless wraps were landing about 50%
  too early. Now 0.4. Only affects installs without a native canvas.
