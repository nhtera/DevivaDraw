...
- `text-editor-overlay.tsx`'s current position-tracking (a fixed debug-poll interval re-render in the dev harness) drifts during a live pan while editing text — it needs to reposition from the actual render loop or a real camera-change subscription instead. Its `canvasBackgroundColor` prop (currently hardcoded white, to avoid double-vision against the last-committed canvas render underneath) must also follow the theme provider built in this phase rather than staying a fixed color.
